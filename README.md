...
Note: 
1.SSH should be enabled on the interface of the firewall which you using to schedule the reboot and for the security purposes allow ssh for only trusted IP's in SonicWall WAN>WAN ACL.
...
9.For the SMTP only basic auth account is supported, enable the App passwords for your email account and get the credentials and upload in the app which saves the configuration in the backend>config.json file,so if you are sharing with your friends directly share my github repo url and clone it by without getting your smtp passwords gettting shared.
10.The whole pacakges were audited with PIP audit and all the version are up to date, if your organisation worried about the zero day attacks put the app in the DMZ zone and close the wan>lan & dmz>lan rules.
10.This app strictly uses only the non-config mode of the sonicWalls, so no need to worry about the config changes.
---
